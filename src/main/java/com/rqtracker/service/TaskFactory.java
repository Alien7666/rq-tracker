...
public final class TaskFactory {
    ...
    public static List<TaskDef> versionDevTasks(int vIdx) {
        return versionDevTasks(vIdx, null);
    }

    /**
     * 修復問題型 RQ（rq.isBugFix()==true）時跳過「測試報告」步驟（dev_07b）。
     */
    public static List<TaskDef> versionDevTasks(int vIdx, RQData rq) {
        String p = "v" + vIdx + "_dev_";
        boolean bugFix = rq != null && rq.isBugFix();
        List<TaskDef> tasks = new ArrayList<>();
        tasks.add(TaskDef.builder(p + "03", "撰寫程式（含 SQL）").step("03").build());
        tasks.add(TaskDef.builder(p + "04", "本機測試程式").step("04").build());
        tasks.add(TaskDef.builder(p + "05", "Fortify").step("05").sub("靜態掃描").build());
        tasks.add(TaskDef.builder(p + "05b", "製版").step("06").build());
        tasks.add(TaskDef.builder(p + "06", "上 SIT").step("07").sub("部署").build());
        tasks.add(TaskDef.builder(p + "07", "SIT 測試").step("08").build());
        if (!bugFix) {
            tasks.add(TaskDef.builder(p + "07b", "測試報告").step("09").build());
        }
        tasks.add(TaskDef.builder(p + "08", "ZAP 動態掃描").step("10").build());
        tasks.add(TaskDef.builder(p + "09", "匯出弱點報告").step("11").build());
        tasks.add(TaskDef.builder(p + "09b", "滲透測試報告").step("12").build());
    ...
    public static List<TaskDef> versionSVNTasks(int vIdx, String vName, RQData rq, String svnRoot) {
        ...
        // svn_004（修復問題型跳過）
        if (!bugFix) {
            tasks.add(TaskDef.builder(p + "004", "放入測試報告")
                .folder(SVN + "004_測試報告\\")
                .filename("中華郵政_網路郵局系統_測試報告_" + rqNum + "_" + vid + ".xlsx")
                .build());
        }

        tasks.add(TaskDef.builder(p + "zap1", "ZAP 中華郵政弱點掃描")
        ...
        tasks.add(TaskDef.builder(p + "zap1_excl", "ZAP 弱點掃描排除說明（交付客戶副本）")
            .folder(ensureSlash(svnRoot) + "9_文件\\99_共用文件\\19.弱掃及滲透測試(ZAP)\\1_中華郵政\\" + PathUtils.sbomSystemFolder(vid) + "\\" + td + "\\")
            .filename("ZAP弱點掃描排除說明_" + vid + ".xlsx")
            .warn("當天產生")
            .build());

        tasks.add(TaskDef.builder(p + "zap1_pentest", "滲透測試報告（交付客戶副本）")
            .folder(ensureSlash(svnRoot) + "9_文件\\99_共用文件\\19.弱掃及滲透測試(ZAP)\\1_中華郵政\\" + PathUtils.sbomSystemFolder(vid) + "\\" + td + "\\")
            .filename("滲透測試_" + vid + "_" + td + ".docx")
            .warn("當天產生")
            .build());

        tasks.add(TaskDef.builder(p + "zap1_pentestcsv", "滲透測試過程報告（交付客戶副本）")
            .folder(ensureSlash(svnRoot) + "9_文件\\99_共用文件\\19.弱掃及滲透測試(ZAP)\\1_中華郵政\\" + PathUtils.sbomSystemFolder(vid) + "\\" + td + "\\")
            .filename("滲透測試過程報告_" + vid + "_" + td + ".csv")
            .warn("當天產生")
            .build());

        tasks.add(TaskDef.builder(p + "zap2", "ZAP 內部系統盤點弱點掃描")
        ...
        tasks.add(TaskDef.builder(p + "sbom", "SBOM 系統盤點更新")
    ...
    // ──────────────────────────────────────────────
    // 6. sharedSVNTasks(rq) — 共用 SVN 任務（5 個）
    // ──────────────────────────────────────────────
    ...
    public static List<TaskDef> sharedSVNTasks(RQData rq, String svnRoot) {
        ...
        boolean bugFix = rq.isBugFix();

        List<TaskDef> tasks = new ArrayList<>();

        tasks.add(TaskDef.builder("svn_003_doc", "廠商交付程式說明 更新")
        ...
        // 修復問題型跳過程式更新紀錄單與程式測試報告單
        if (!bugFix) {
            tasks.add(TaskDef.builder("svn_003_updrec", "程式更新紀錄單 更新")
                .folder(SVN)
                .filename(proj + "_" + rqNum + "_開放系統程式更新紀錄單_Vx.x.docx")
                .checkModTime()
                .build());

            tasks.add(TaskDef.builder("svn_003_testrec", "程式測試報告單 更新")
                .folder(SVN)
