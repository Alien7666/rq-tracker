...
import com.rqtracker.ui.dialog.FolderListDialog;
...
public class RQContentPanel extends VBox {
    ...
    private void build() {
        ...
        Label progressInfo = new Label(
            "完成 " + prog.done() + " / " + prog.total() + " 項（不含選填）");
        progressInfo.getStyleClass().add("progress-info");

        Button folderBtn = new Button("📂 資料夾清單");
        folderBtn.getStyleClass().add("btn-edit-versions");
        folderBtn.setOnAction(e -> {
            FolderListDialog dlg = new FolderListDialog(
                getScene().getWindow(), dataStore, appConfig, rqId);
            dlg.setOnCheckChanged(() -> {
        ...
        Button editVerBtn = new Button("✏ 編輯版本");
        editVerBtn.getStyleClass().add("btn-edit-versions");
        editVerBtn.setOnAction(e -> { if (onEditVersions != null) onEditVersions.accept(rqId); });

        Label pctLabel = new Label(pct + "%");
        pctLabel.getStyleClass().add("pct-label");

        HBox topRight = new HBox(12, folderBtn, editVerBtn, pctLabel);
        topRight.setAlignment(Pos.CENTER_RIGHT);

        Region topSpacer = new Region();
        HBox.setHgrow(topSpacer, Priority.ALWAYS);

        HBox topRow = new HBox(topSpacer, topRight);
        topRow.setAlignment(Pos.CENTER_LEFT);
        HBox topRowFull = new HBox(progressInfo, topSpacer, topRight);
        topRowFull.setAlignment(Pos.CENTER);

        getChildren().add(topRowFull);

        // ── 進度條 ────────────────────────────────────────────────
        ProgressBar bar = new ProgressBar(pct / 100.0);
        bar.getStyleClass().add("rq-progress-bar");
        bar.setMaxWidth(Double.MAX_VALUE);
        if (pct >= 100) bar.getStyleClass().add("done");
        getChildren().add(bar);
    ...
    private VBox buildNoteBlock(RQData rq) {
        ...
        VBox noteBody = new VBox(noteArea);
        noteBody.getStyleClass().add("note-body");
        noteBody.setPadding(new Insets(0, 12, 8, 12));
    ...
    private VBox buildVersionCard(RQData rq, int vIdx, String dlRoot, String svnRoot) {
        ...
        Label vProgText = new Label(vProg.done() + "/" + vProg.total());
        vProgText.getStyleClass().add("version-prog-text");
        ...
        HBox vBody = new HBox(0, devSection, svnSection, delSection);
        vBody.getStyleClass().add("version-body");
        HBox.setHgrow(devSection, Priority.ALWAYS);
        HBox.setHgrow(svnSection, Priority.ALWAYS);
        HBox.setHgrow(delSection, Priority.ALWAYS);
